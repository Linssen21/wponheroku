		...
		<?php $this->parent->get_widget_title_html() ; ?>
			...
			<?php
				foreach($images as $image){
					...
					//  die('dfdf');
					?>
					<a href="<?php echo esc_html( $image['url'] ); ?>" title="<?php echo esc_html( $image['title'] ); ?>" <?php echo $this->parent->get_render_attribute_string( 'jg_item_inner' ); ?> <?php echo $this->parent->get_render_attribute_string( 'link' ); ?> data-elementor-lightbox-title="<?php echo esc_html( $img_cap ); ?>">
						<?php echo wp_get_attachment_image( $image['id'], $image_size, false, [ 'title' => $image['title'],'class'	=>	'ae-jg-item' ] ); ?>
						<div class="ae-grid-overlay <?php echo esc_html( $animation ); ?>">
							<div class="ae-grid-overlay-inner">
								<div class="ae-icon-wrapper">
									<?php if ( ! empty( $icon ) ) { ?>
